...
public class Problem18 extends Problem {
    ...
    public static int highestPath(int[][] triangle) {//triangular array
        int[][] highSums = new int[triangle.length][];
        int x, y;

        for (x = 0; x < highSums.length; x++)
            highSums[x] = new int[triangle[x].length];//just makes highSums a triangular array

        x = triangle.length - 1;
        for (y = 0; y < triangle[x].length; y++)
            highSums[x][y] = triangle[x][y];//fills bottom row of highSums

        for (x = triangle.length - 2; x >= 0; x--) {
            for (y = 0; y < triangle[x].length; y++) {
                highSums[x][y] = triangle[x][y] + Math.max(highSums[x + 1][y],
                        highSums[x + 1][y + 1]);
            }
        }//^^^ the actual logic ^^^
